...
# Build into a FRESH temp file, verify, then swap. Attaching the LIVE db
# let CREATE TABLE out.pois collide with the old 9-column schema
# ("table pois already exists"); the enrichment silently never landed and
# the verification SELECTs then reported the OLD table as a success.
TMP="$BASE/data-api/pois.build.db"
DUCK="$BASE/duckdb"
...
SELECT
  ...
  map_extract(tags, 'phone')[1]         AS phone,
  map_extract(tags, 'website')[1]       AS website,
  map_extract(tags, 'opening_hours')[1] AS opening_hours,
  map_extract(tags, 'cuisine')[1]       AS cuisine,
  map_extract(tags, 'wheelchair')[1]    AS wheelchair,
...
SELECT
  ...
  brand, phone, website, opening_hours, cuisine, wheelchair,
...
"$BASE/duckdb" -c "ATTACH '$TMP' AS o (TYPE SQLITE); " >/dev/null 2>&1 || true
python3 - "$TMP" <<'PY'
import sqlite3, sys
...
db.close()
PY
# Only replace the live db if the enrichment really landed — the whole
# point of this rebuild. Anything less and the API keeps serving the old one.
python3 - "$TMP" <<'GUARD'
import sqlite3, sys
cols = {r[1] for r in sqlite3.connect(sys.argv[1]).execute("PRAGMA table_info(pois)")}
need = {"phone", "website", "opening_hours", "cuisine", "wheelchair"}
missing = need - cols
if missing:
    sys.exit("ABORT: enriched columns missing: %s" % sorted(missing))
print("enriched columns present:", sorted(need))
GUARD

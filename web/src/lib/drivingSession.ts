...
export function createDrivingSession(route: { distanceM: number; durationS: number }, now = Date.now()): DrivingSession {
  if (!Number.isFinite(route.distanceM) || route.distanceM <= 0) {
    return { status: 'idle', mode: 'game', startedAt: null, lastInputAt: null, elapsedMs: 0, durationMs: 0, progress: 0, distanceM: 0, speedMps: 0, lateral: 0 }
  }
  ...
  return { status: 'ready', mode: 'game', startedAt: null, lastInputAt: null, elapsedMs: 0, durationMs, progress: 0, distanceM: route.distanceM, speedMps: 0, lateral: 0 }
}

export function startDriving(session: DrivingSession, now = Date.now()): DrivingSession {
  if (session.status !== 'ready') return session
  return { ...session, status: 'running', startedAt: now, lastInputAt: now }
}
...
export function pauseDriving(session: DrivingSession, now = Date.now()): DrivingSession {
  ...
  return { ...session, status: 'paused', startedAt: null, lastInputAt: null, elapsedMs, progress: progressForElapsed(elapsedMs, session.durationMs) }
}
...
export function finishDriving(session: DrivingSession, now = Date.now()): DrivingSession {
  ...
  return { ...current, status: 'finished', startedAt: null, lastInputAt: null, elapsedMs: Math.min(current.elapsedMs, current.durationMs), progress: 1 }
}
